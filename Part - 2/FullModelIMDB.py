...
if __name__ == '__main__':
    ...
    with psycopg2.connect(host=host, dbname=dbname, user=user, password=password, port=port) as connectionToDB:
        ...
        cursor.execute('''CREATE TABLE Title_Actor (actor INT,
                                                    title INT,
        ...
        cursor.execute('''CREATE TABLE Title_Writer ( writer INT,
                                                      title INT,
        ...
        cursor.execute('''CREATE TABLE Title_Director ( director INT,
                                                        title INT,
        ...
        cursor.execute('''CREATE TABLE Title_Producer ( producer INT,
                                                        title INT,
        ...
        cursor.execute(
            " INSERT INTO Title_Actor(title, actor) (SELECT DISTINCT TitleID, PersonID FROM TitlePrincipals WHERE category = 'actor'"
        ...
        cursor.execute(
            " INSERT INTO Title_Writer(title, writer) (SELECT DISTINCT TitleID, PersonID FROM TitlePrincipals WHERE category = 'writer'"
            " AND EXISTS (SELECT * FROM Title WHERE Title.id = TitlePrincipals.TitleID)"
            " AND EXISTS (SELECT * FROM Member WHERE Member.id = TitlePrincipals.PersonID));")

        cursor.execute(
            " INSERT INTO Title_Director(title, director) (SELECT DISTINCT TitleID, PersonID FROM TitlePrincipals WHERE category = 'director'"
            " AND EXISTS (SELECT * FROM Title WHERE Title.id = TitlePrincipals.TitleID)"
            " AND EXISTS (SELECT * FROM Member WHERE Member.id = TitlePrincipals.PersonID));")

        cursor.execute(
            " INSERT INTO Title_Producer(title, producer) (SELECT DISTINCT TitleID, PersonID FROM TitlePrincipals WHERE category = 'producer'"
            " AND EXISTS (SELECT * FROM Title WHERE Title.id = TitlePrincipals.TitleID)"
            " AND EXISTS (SELECT * FROM Member WHERE Member.id = TitlePrincipals.PersonID));")
